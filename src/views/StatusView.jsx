...
export default function StatusView() {
  ...
  }, [player, isMeditating, auraColorStr, auraHex]); 

  if (!player) return null;

  return (
    // 🌟 修改 1：外層改成透明背景 (bg-transparent)
    <div className="h-full w-full relative bg-transparent overflow-hidden">

      {/* =========================================
          🌟 修改 2：打坐氛圍遮罩 (取代原本厚重的背景與粒子)
          這會讓底層 PlayingStage 的網格透上來，但在打坐時會覆蓋一層紫色光暈
          ========================================= */}
      <div 
        className={`absolute inset-0 pointer-events-none transition-all duration-1000 z-0
          ${isMeditating ? 'bg-[#9B5CFF]/10' : 'bg-transparent'}
        `}
        style={{ 
          background: isMeditating 
            ? 'radial-gradient(circle at center, rgba(155,92,255,0.15) 0%, transparent 80%)' 
            : 'radial-gradient(circle at center, rgba(0,229,255,0.05) 0%, transparent 80%)'
        }}
      ></div>

      {/* =========================================
          🔥 滾動內容層 (Z-Index 確保在遮罩之上)
          ========================================= */}
      <div className="absolute inset-0 w-full h-full flex flex-col items-center pt-[2cqw] px-[6cqw] overflow-y-auto no-scrollbar pb-[10cqw] z-10">
        ...
        {/* 3. 修仙資訊面板 */}
        <div className="w-full flex flex-col gap-[2.5cqw] mb-[6cqw] pt-[2cqw] bg-white/[0.02] rounded-xl p-[4cqw] border border-white/5 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] z-10 backdrop-blur-sm">
