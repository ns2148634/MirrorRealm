...
SELECT * FROM (VALUES
  -- ── 凡人期 ──
  ('勞作', '田間農活',   10, 0, 'mortal',
   ARRAY['此地有農田，可協助耕作換取報酬。','農人忙碌，似乎需要幫手。'],
   ARRAY['勞動']),
  ('勞作', '碼頭搬運',   12, 0, 'mortal',
   ARRAY['碼頭船隻往來，有搬運工作可做。','船東正在招募短工。'],
   ARRAY['勞動']),
  ('勞作', '市集攤位',    8, 0, 'mortal',
   ARRAY['市集熱鬧，可擺攤或幫忙招呼客人。','攤主需要人手協助。'],
   ARRAY['勞動']),
  ('見聞', '茶館說書',    5, 0, 'mortal',
   ARRAY['茶館內說書人正在講述江湖奇聞。','說書人口若懸河，或有奇聞。'],
   ARRAY['見聞']),
  ('見聞', '老者問道',    5, 0, 'mortal',
   ARRAY['路旁有位老者，眼神深邃，似有所悟。','老者正在打坐，周身隱有靈氣波動。'],
   ARRAY['見聞']),
  ('見聞', '廟宇碑文',    5, 0, 'mortal',
   ARRAY['廟宇前有古老碑文，字跡模糊卻隱有玄機。','碑文刻有前人修行感悟。'],
   ARRAY['見聞']),
  ('衝突', '街頭惡霸',   15, 5, 'mortal',
   ARRAY['幾個地痞正在欺壓百姓。','有人被攔路打劫，呼救聲傳來。'],
   ARRAY['衝突']),
  ('衝突', '門派糾紛',   10, 3, 'mortal',
   ARRAY['兩個小門派的弟子正在爭執。','附近傳來劍氣波動，有人在比武。'],
   ARRAY['衝突']),
  -- ── 通用 ──
  ('拾荒', '廢墟遺址',    8, 0, 'both',
   ARRAY['廢墟中或有遺落的修仙物品。','此地昔日似為修士聚居之所，或有殘留。'],
   ARRAY['拾取']),
  ('拾荒', '荒野草叢',    6, 0, 'both',
   ARRAY['草叢間有異物閃光。','感應到微弱的靈氣波動，似有靈草。'],
   ARRAY['拾取']),
  ('拾荒', '山洞入口',   10, 0, 'both',
   ARRAY['山洞深處或藏有寶物。','洞口有獸跡，但靈氣充盈。'],
   ARRAY['拾取']),
  -- ── 修仙期 ──
  ('妖獸', '山野妖狐',    0, 10, 'immortal',
   ARRAY['一隻妖狐盤踞此地，靈氣濃郁。','感應到妖獸氣息，小心戒備。'],
   ARRAY['戰鬥']),
  ('妖獸', '溪邊蛟蛇',    0, 12, 'immortal',
   ARRAY['水邊有蛟蛇盤臥，靈氣蜷繞。','濃厚的妖氣從水中升騰而起。'],
   ARRAY['戰鬥']),
  ('妖獸', '林中魔熊',    0, 15, 'immortal',
   ARRAY['林中有魔熊出沒，氣勢兇猛。','巨大的腳印延伸至林中深處。'],
   ARRAY['戰鬥']),
  ('機緣', '古樹靈穴',    5, 0, 'immortal',
   ARRAY['千年古樹下有靈穴，靈氣充沛。','古樹枝椏間隱有靈光閃動。'],
   ARRAY['機緣']),
  ('機緣', '石碑法訣',    5, 0, 'immortal',
   ARRAY['荒野中有一塊石碑，刻有殘缺功法。','石碑上的符文在月光下發出幽光。'],
   ARRAY['機緣'])
) AS v(node_type, node_name, sp_cost, hp_cost, phase, mud_texts, lbs_categories)
